...
# ##########################################################
def colorHistOnHeight(N, bins, patches):
...
# ##########################################################
def normBarHeight(bins, patches, rotated=False):
    ##widths = np.diff(bins)
    for item in patches:
        if not rotated:
            item.set_height(item.get_height())
        else:
            item.set_width(item.get_width())
    return
...
def genDensityPlot(x, y, mass, pf, z, filename, xaxislabel, normByPMass=True):
    ...
    if normByPMass:
        ...
        normBarHeight(bins, patches, rotated=True)
    ...
    colorHistOnHeight(N, bins, patches) # Normalizes colors to our colorbar
...
for indx, z in enumerate(files):
    ...
    genDensityPlot(spZ, # x-axis
                   ...
                   spMass, spPF, z,
                   "Z-vs-Z_pri-MassHistLog-noNorm", "log $Z_{\odot}$", normByPMass=False)
    ...
    genDensityPlot(np.ma.masked_invalid(spZ / f_pol), # x-axis
                   np.ma.masked_invalid(spPZ / spZ),  # y-axis
                   spMass, spPF, z,
                   "Z-f_pol-vs-Z_pri-MassHistLog-noNorm", "log $(Z_{\odot}/f_{pol})$")
