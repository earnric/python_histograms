...
#  30 Dec 2015 - Updated to plot vs element abundances
...
def genDensityPlot(x, y, mass, pf, z, filename, xaxislabel, yaxislabel, normByPMass=True):
    ...
    # Only care about low metallicity values along x-axis
    mass = mass[x < 1E-3]
    pf = pf[x < 1E-3]
    y=y[x < 1E-3]
    x=x[x < 1E-3]

    # FuncFormatter class instance defined above
...
Fe_sun  = 0.0013  # solar mass fraction
Fe_star = 0.00024 # 25 M_sun Pop III SN yield, solar units 
C_sun  = 0.0024   # solar mass fraction
C_star = 0.28     # 25 M_sun Pop III SN yield, solar units 
N_sun  = 0.00071  # solar mass fraction
N_star = 0.0045   # 25 M_sun Pop III SN yield, solar units 
        
for indx, z in enumerate(files):
    print ("Loading data for z={:.1f}".format(float(z)))
    ...
    print ("Generating PDFs for z=%s" % z)
    ...
    f_pol = np.array((1.0 - spPF))
    if len(f_pol[f_pol <= 0.0]):
        print("Number of purely Pop III stars = {}".format(len(f_pol[f_pol <= 0.0])))
        #f_pol = np.ma.masked_less_equal((1.0 - spPF), 0.0)  # The polluted fraction
        f_pol[f_pol <= 0] = 5.0e-9 # This is the value of sp's with Z or ZP ~ 0.0

    temp = (spZ / f_pol) - (spPZ/f_pol)
    if len(temp[temp <= 0.0]):
        print("We have some zero values for (spZ / f_pol) - (spPZ/f_pol) {}".format(len(temp[temp <= 0.0])))

    genDensityPlot(
#                   (spZ / f_pol) * Fe_sun - (spPZ/f_pol) * Fe_star + 10**-8.2, # x-axis
                   (spPZ / f_pol) * Fe_star, # + 10**-8.2, # x-axis
                   (spPZ / f_pol) * C_star,  # y-axis
                   spMass, spPF, z,
                   "Z-ZP_f-vs-ZP_f_pol-FeVsCstar",
#                   "log $((Z_{\odot}-ZP_{\odot})/f_{pol})$","log $(ZP_{\odot}/f_{pol})$")
                   "log $((Fe_{\odot}-Fe_{Pri, \odot})/f_{pol})$","log $(Fe_{Pri, \odot}/f_{pol})$")

    genDensityPlot(
#                   (spZ / f_pol) * Fe_sun - (spPZ/f_pol) * Fe_star + 10**-8.2, # x-axis
                   (spPZ / f_pol) * Fe_star, # + 10**-8.2, # x-axis
                   (spZ / f_pol) * C_sun,  # y-axis
                   spMass, spPF, z,
                   "Z-ZP_f-vs-Z_f_pol-FeVsCsun",
#                   "log $((Z_{\odot}-ZP_{\odot})/f_{pol})$","log $(Z_{\odot}/f_{pol})$")
                   "log $((Fe_{\odot}-Fe_{Pri, \odot})/f_{pol})$","log $(Fe_{\odot}/f_{pol})$")

    ## genDensityPlot(
    ##                (spZ  - spPZ) + 10**-8.2, # x-axis
    ##                (spPZ),  # y-axis
    ##                spMass, spPF, z,
    ##                "Z-ZP-vs-ZP-2-fixed",
    ##                "log $(Z_{\odot}-ZP_{\odot})$","log $(ZP_{\odot})$",False)
    ...
    ##                spMass, spPF, z,
    ##                "Z-ZP-vs-Z-2-fixed",
    ##                "log $(Z_{\odot}-ZP_{\odot})$","log $(Z_{\odot})$",False)
