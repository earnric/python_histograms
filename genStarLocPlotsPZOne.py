...
for locF,massF,ZF,ppfF,pzfF in zip(locFiles,massFiles,ZFiles,ppfFiles,ppzFiles):
    ...
    for indx, poi in enumerate(poisFiles):
        ...
        rng1 = (pzfFilt < 1.e-5)
        rng2 = ((pzfFilt >= 1.e-5) & (pzfFilt < 1.e-3))
        rng3 = ((pzfFilt >= 1.e-3) & (pzfFilt < 1.e-1))
        rng4 = (pzfFilt >= 1.e-1)
        p1=np.log10(pzfFilt[rng1])
        p2=np.log10(pzfFilt[rng2])
        p3=np.log10(pzfFilt[rng3])
        p4=np.log10(pzfFilt[rng4])
        ...
        ax1.scatter(xcoord[rng4], ycoord[rng4], s=massFilt[rng4]/dotNorm, c=p4, cmap=cmap,vmin=-7.5, vmax=0)
        ax1.scatter(xcoord[rng3], ycoord[rng3], s=massFilt[rng3]/dotNorm, c=p3, cmap=cmap,vmin=-7.5, vmax=0)
        ax1.scatter(xcoord[rng2], ycoord[rng2], s=massFilt[rng2]/dotNorm, c=p2, cmap=cmap,vmin=-7.5, vmax=0)
        ...
        #ax1.set_title('$Z_{P,\odot}$',size=20)
        ...
        ax5.set_ylabel('$log_{10}\; Z_{P, \odot}$', size=34)
        plt.subplots_adjust(left=0.2, bottom=0.15, right=0.80, top=0.85, wspace=.15, hspace=.15)
        plt.savefig("SP_PZ_locs_z=%.1lf-%i.pdf"%(z,indx))
#        plt.show()
        print( "Files saved: SP_PZ_locs_z=%.1lf-%i.pdf"%(z,indx))
        print( "\n")
        plt.close()
