...
for locF,massF,ZF,ppfF,pzfF in zip(locFiles,massFiles,ZFiles,ppfFiles,ppzFiles):
    ...
    print("Looking for file coord files: "+spPosExp)
    ...
    for indx, poi in enumerate(poisFiles):
        ...
        locs = np.loadtxt(locF,skiprows=1)  # loc, mass, etc files have header lines... 
        ...
        ax1.text(xpos,ypos,'$Z_{P, \odot} <\, 10^{-5}$',
                 bbox=bbox, fontsize=20)
        ax2.text(xpos,ypos,'$10^{-5} \leq\, Z_{P, \odot}\, <\, 10^{-3}$',
                 bbox=bbox,fontsize=20)
        ax3.text(xpos,ypos,'$10^{-3} \leq\, Z_{P, \odot}\, <\, 10^{-1}$',
                 bbox=bbox,fontsize=20)
        ax4.text(xpos,ypos,'$10^{-1} \leq\, Z_{P, \odot}$',
                 bbox=bbox,fontsize=20)
        ...
        ax5.set_ylabel('$log\; Z_{P, \odot}$', size=34)
        plt.subplots_adjust(left=0.15, bottom=0.1, right=0.84, top=0.95, wspace=.05, hspace=.05)
        plt.savefig("SP_PZ_locs4Panel_z=%.1lf-%i.pdf"%(z,indx))
#        plt.show()
        print( "Files saved: SP_PZ_locs4Panel_z=%.1lf-%i.pdf"%(z,indx))
        print( "\n")
        plt.close()
